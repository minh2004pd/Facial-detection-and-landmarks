...
def pipeline(xml_file_path="./data/labels_ibug_300W_test.xml", output_dir="image", pred=None):
    ...
    for idx, filename in enumerate(root[2]):
        ...
        image_pred = pred[idx][0]   # Adjust index based on your prediction structure
        ...
        image1 = np.array(Image.open(file_path))
        if (image1.ndim == 2):
            continue
        ...
        # print(points)
        # box1 = []
        # box1.append(filename[0].attrib)

        cropped_image, top, left = crop_image(image, filename[0].attrib)
        ...
        #     y_max = b[3]
        #     box = [
        #         [y_min, x_min, x_max - x_min, y_max - y_min] 
        #     ]

        #     box1.append(filename[0].attrib)

        #     cropped_image, top, left = crop_image(image, box1[idx])
    ...
    #     file_path = image_node.attrib['file']
    ...
    #     # Crop the image (assuming the first box for simplicity)
    #     box = image_node.find('box')
    #     cropped_image = crop_image(image, box)
        
    #     # Apply predictions to the cropped image
    #     processed_image = apply_predictions(cropped_image)
...
def main(cfg: DictConfig) -> None:
    ...
    model = ResNet.load_from_checkpoint("/data/hpc/minhdd/filter/results_resnet501/checkpoints/epoch=183-mae=0.02.ckpt", strict=False)
